...
Book.create = (newBook, result) => {
    console.log(newBook);
    sql.query("INSERT INTO books (title, description, image) VALUES (?,?,?)",
        [newBook.title, newBook.description, newBook.image], (err,
            res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
...
Book.updateById = (id, Book, result) => {
    sql.query(
        ...
        [Book.title, Book.description, Book.image, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
